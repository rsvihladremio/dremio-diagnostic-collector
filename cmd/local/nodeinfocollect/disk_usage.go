...
func RunCollectDiskUsage(c *conf.CollectConf, hook shutdown.CancelHook) error {
	...
	if err != nil {
		return fmt.Errorf("unable to create diskusage.txt: %w", err)
	}
	defer func() {
		if err := diskWriter.Sync(); err != nil {
			simplelog.Warningf("unable to sync the os_info.txt file: %v", err)
		}
		if err := diskWriter.Close(); err != nil {
			simplelog.Warningf("unable to close the os_info.txt file: %v", err)
		}
	}()
	err = ddcio.Shell(hook, diskWriter, "df -h")
	if err != nil {
		simplelog.Warningf("unable to read df -h: %v", err)
	}
	...
	if strings.Contains(c.NodeName(), "dremio-master") {
		...
		if err != nil {
			return fmt.Errorf("unable to create rocksdb_disk_allocation.txt: %w", err)
		}
		defer func() {
		...
		if err != nil {
			simplelog.Warningf("unable to write du -sh to rocksdb_disk_allocation.txt: %v", err)
		}
